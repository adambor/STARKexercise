...
export class NumCompare {
    ...
    prove(num1: bigint, num2: bigint, bitLength: number, runChecks?: boolean): {
        ...
        const stark = new Stark(this.field, this.byteLength, this.expansionFactor, this.securityLevel, 5, bitLength+1, 3, this.fieldGenerator, this.foldingFactor);

        const powersOf2 = this.field.getPowerSeries(2n, bitLength+1).toValues().reverse();

        const powersOf2Poly = Polynomial.interpolateDomain(stark.omicronDomain.slice(0, bitLength+1), powersOf2, this.field);
        ...
        const num1Binary = num1.toString(2).padStart(bitLength+1, "0");
        const num2Binary = num2.toString(2).padStart(bitLength+1, "0");
        ...
        const boundaryConditions: BoundaryConditions = new Map([
            ...
            [3, [{cycle: 0, value: 0n}, {cycle: bitLength, value: num2}]],
